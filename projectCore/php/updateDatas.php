<?php
    ...
    if($requestMethod == 'GET' && $requestRessource == 'match-coming-list') {
        $request_sql = "SELECT m.id, m.date, m.start_hour, m.duration, m.name, m.address, m.nb_player, c.name, s.name, s.picture FROM participe_a p JOIN match_event m ON p.id = m.id JOIN city c ON c.insee = m.insee JOIN sports s ON s.id = m.id_sports WHERE p.mail =:m_mail";
        $query = $db->prepare($request_sql);
        $query->execute(array(
